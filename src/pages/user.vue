<template>
  <div class="px-15">
    <div class="font-bold text-4xl py-10">Data Pegawai</div>
    <div class="py-15">
  ...
  <tbody>
    <tr v-for="data in posts" :key="data.id">
      ...
      <td>
        {{ data.text }}
        <button class="edit-btn" @click="edit(data.id)">Edit</button>
        <button class="delete-btn" @click="destroy(data.id)">Delete</button>
      </td>
    </tr>
  </tbody>
</table>
      ...
      <div class="flex justify-center items-center top-[30%] fixed sticky-0 left-[40%]">
        <div class="md:w-auto relative flex flex-col justify-center items-center bg-white p-8">
          ...
          <div class="mt-2 flex flex-col space-y-2">
            ...
            <input type="text" class="border-2 py-2 px-1 border-2 w-full text-gray-800 text-base leading-4 text-left mt-2 focus:outline-2" rows="8" cols="8" placeholder="Jabatan" v-model="data.number">
          </div>
